...
namespace reddecoder {
    ...
    VideoCodecError Ndk_MediaCodecVideoDecoder::feed_decoder(const Buffer *buffer,
        ...
        while ((offset < buffer->get_size() || buffer->get_size() == 0) &&
            ...
            if (buf_idx < 0) {
                // 获取输入缓冲区（超时30ms）
                buf_idx = AMediaCodec_dequeueInputBuffer(media_codec_.get(),AMC_INPUT_TIMEOUT_US);
                if (buf_idx < 0) {
            ...
            size_t buf_size;
            // 拷贝数据到MediaCodec输入缓冲区
            uint8_t *buf =AMediaCodec_getInputBuffer(media_codec_.get(), buf_idx, &buf_size);
    ...
    VideoCodecError Ndk_MediaCodecVideoDecoder::drain_decoder() {
        if (!is_eof_state_) {
            ...
            // 获取输出缓冲区（超时10ms）
            auto status = AMediaCodec_dequeueOutputBuffer(media_codec_.get(), &info,drain_timeout_us);
            // 处理输出数据
            if (status >= 0) {
