...
// 准备视频处理流程（调用时机：播放器初始化/格式变更）
    RED_ERR CVideoProcesser::Prepare(sp<MetaData> &metadata) {
        ...
        // 1. 创建帧队列（默认大小：VIDEO_PICTURE_QUEUE_SIZE_DEFAULT）
    ...
    RED_ERR CVideoProcesser::Init() {
        ...
        // 4. 注册解码回调（this实现on_decoded_frame接口）
